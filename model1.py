...
def model_call():
    ...
    for symptom in constant_symptoms:
        if symptom in patients_symptoms:
            value_to_pass.append(1)
        else:
            value_to_pass.append(0)

    url =  r'C:\Users\HP\Desktop\minor_project\dataset.csv'
    df = pd.read_csv(url, encoding='latin-1')
    predicted_disease = model.predict(value_to_pass)[0]
    predicted_precaution = df[df['disease'] == predicted_disease]['precautions'].values[0]
    return [predicted_disease, predicted_precaution]
